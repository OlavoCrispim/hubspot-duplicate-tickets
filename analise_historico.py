...
ID_ETAPA_RESOLVIDO = "1067263045" # O ID da etapa "Resolvido / Fechado"

# --- Funções Reutilizadas e Adaptadas ---

def find_original_ticket_historico(ticket_a_verificar, properties_to_match):
    """
    Busca por um ticket original mais antigo, ignorando o limite de data.
    """
    search_url = "https://api.hubapi.com/crm/v3/objects/tickets/search"
    headers = {"Authorization": f"Bearer {HUBSPOT_API_KEY}", "Content-Type": "application/json"}

    filters = [
        # O ticket encontrado não pode ser o mesmo que estamos a verificar
        {"propertyName": "hs_object_id", "operator": "NEQ", "value": ticket_a_verificar['id']},
        # O ticket encontrado tem de ser MAIS ANTIGO que o que estamos a verificar
        ...
        # Não considerar tickets que também já estão resolvidos
        {"propertyName": "hs_pipeline_stage", "operator": "NEQ", "value": ID_ETAPA_RESOLVIDO},
    ]

    query = {
        "filterGroups": [{"filters": filters}],
        "sorts": [{"propertyName": "createdate", "direction": "ASCENDING"}], "limit": 1
    }
...
def update_duplicate_ticket_info(duplicate_ticket_id, original_ticket_id):
    ...
    print(f"  -> ATUALIZANDO TICKET #{duplicate_ticket_id} para ser duplicata de #{original_ticket_id}")
    response = requests.patch(update_url, headers=headers, json=payload)
    response.raise_for_status()

def get_all_open_tickets():
    """Busca todos os tickets que não estão resolvidos, lidando com paginação."""
    all_tickets = []
    after = None
    ...
    query = {
        "filterGroups": [{
            "filters": [
                {"propertyName": "hs_pipeline", "operator": "EQ", "value": "732696496"}, # Filtro para pipeline "Atendimento"
                {"propertyName": "hs_pipeline_stage", "operator": "NEQ", "value": ID_ETAPA_RESOLVIDO},
                {"propertyName": "status_da_duplicata", "operator": "NOT_HAS_PROPERTY"} # Ignora tickets que já analisámos
            ]
        }],
        "sorts": [{"propertyName": "createdate", "direction": "DESCENDING"}], # Começa pelos mais recentes
        "properties": ["rc__marca", "rc__tipo_de_solicitacao", "submotivo_do_contato", "e_mail_do_aluno", "createdate"],
        "limit": 100
    }
    ...
    while True:
        ...
        print(f"Buscando uma página de tickets abertos...")
        ...
        if "paging" in data and "next" in data["paging"]:
            ...
            time.sleep(0.5) # Pausa entre as páginas
        else:
            break
...
if __name__ == '__main__':
    print("--- INICIANDO SCRIPT DE ANÁLISE DE TICKETS HISTÓRICOS ---")
    
    tickets_a_processar = get_all_open_tickets()
    total_tickets = len(tickets_a_processar)
    print(f"Total de {total_tickets} tickets abertos encontrados para análise.")

    for index, ticket in enumerate(tickets_a_processar):
        ticket_id = ticket['id']
        properties = ticket.get("properties", {})
        
        print(f"\n({index + 1}/{total_tickets}) Analisando Ticket #{ticket_id}...")
        ...
        try:
            ...
            # Pausa para não sobrecarregar a API do HubSpot
            time.sleep(0.5)

        except Exception as e:
            print(f"  -> Ocorreu um erro ao processar o ticket #{ticket_id}: {e}")

    print("\n--- ANÁLISE DE TICKETS HISTÓRICOS CONCLUÍDA ---")